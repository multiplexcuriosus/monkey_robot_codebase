...
class MoveGroupInterface(object): 
    ...
    def __init__(self):
        ...
        self.m_id_counter = 0 # To disambiguate markers, each one gets an ID. For duplicate markers in MarkerArray only one marker is displayed
        ...
        move_group.set_max_velocity_scaling_factor(0.5) # Scaler for execution of trajectories
    ...
    def createMarker(self,marker_pose,_ns):
        ...
        # If it is not a waypoint being created, it must be one of markers visualizing the last couple of im positions. These must be counted.
        if _ns != "waypoint":
            self.lrimp_count += 1
    ...
    def loadWaypointsFromJSON(self):
        ...
        with open(path_name, 'rb') as f:
                ...
                for pose in loaded_pose_array.poses:
                    self.iface.createMarker(pose,'loaded_wpoints')
                self.iface.markerArrayPub.publish(self.iface.markerArray)
